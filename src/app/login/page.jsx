...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      if (success === 'logged_in') {
        Swal.fire({
          title: "สำเร็จ!",
          text: "เข้าสู่ระบบด้วย Social Login แล้ว",
          icon: "success",
      ...
      Swal.fire({
        title: "ผิดพลาด!",
        text: "ไม่สามารถเข้าสู่ระบบด้วย Social Login ได้",
        icon: "error",
        background: isDarkMode ? '#1a1a2e' : '#ffffff',
        color: isDarkMode ? '#fff' : '#000',
  ...
  }, [searchParams, router, isDarkMode]);
  ...
  async function handleFacebookLogin() {
    try {
      ...
      Swal.fire({
        title: "ผิดพลาด!",
        text: "ไม่สามารถเข้าสู่ระบบด้วย Facebook ได้",
        icon: "error",
        background: isDarkMode ? '#1a1a2e' : '#ffffff',
        color: isDarkMode ? '#fff' : '#000',
        confirmButtonColor: '#ef4444'
  ...
  const bgGradient = isDarkMode 
    ...
    : "from-purple-50 via-pink-50 to-cyan-50";
    ...
    <div className={`min-h-screen bg-gradient-to-br ${bgGradient} flex items-center justify-center p-4 relative overflow-hidden transition-all duration-500`}>
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[...Array(15)].map((_, i) => (
          <div
            key={i}
            className={`absolute w-1.5 h-1.5 ${isDarkMode ? 'bg-purple-400' : 'bg-purple-500'} rounded-full ${isDarkMode ? 'opacity-20' : 'opacity-40'} animate-float`}
            style={{
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className={`${cardBg} rounded-3xl shadow-2xl ${isDarkMode ? 'border border-white/20' : 'border border-gray-200/50'} p-8 transform transition-all duration-500 hover:scale-105`}>
          ...
          <div className="text-center mb-8">
            ...
            <h1 className={`text-3xl font-bold ${textColor} mb-2 bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent`}>
              เข้าสู่ระบบ
            </h1>
            <p className={isDarkMode ? "text-gray-300" : "text-gray-600"}>ยินดีต้อนรับกลับมา</p>
          </div>
          ...
          <form onSubmit={onSubmit} className="space-y-6">
            ...
            <div className="relative group">
              <div className={`absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none transition-colors duration-300 ${
                focusedField === 'email' ? 'text-purple-400' : isDarkMode ? 'text-gray-400' : 'text-gray-500'
              }`}>
              ...
              />
              <div className={`absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-purple-500 to-cyan-500 transform transition-all duration-300 ${
            ...
            <div className="relative group">
              <div className={`absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none transition-colors duration-300 ${
                focusedField === 'password' ? 'text-purple-400' : isDarkMode ? 'text-gray-400' : 'text-gray-500'
              }`}>
            ...
            <div className="text-right">
              <a href="/forgot-password" className={`text-sm ${isDarkMode ? 'text-purple-400 hover:text-purple-300' : 'text-purple-600 hover:text-purple-500'} transition-colors duration-300 underline decoration-wavy`}>
                ลืมรหัสผ่าน?
              </a>
            </div>
          ...
          <div className="mt-6">
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className={`px-3 bg-transparent ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>หรือ</span>
              </div>
            </div>
          ...
          <div className="mt-8 text-center">
            <p className={isDarkMode ? "text-gray-400" : "text-gray-600"}>
              ยังไม่มีบัญชี?{" "}
              <a href="/register" className={`${isDarkMode ? 'text-purple-400 hover:text-purple-300' : 'text-purple-600 hover:text-purple-500'} transition-colors duration-300 underline decoration-wavy`}>
                สมัครสมาชิก
              </a>
            </p>
          </div>
        ...
        {/* Decorative Elements */}
        <div className={`absolute -top-4 -right-4 w-24 h-24 bg-gradient-to-r from-purple-500 to-cyan-500 rounded-full ${isDarkMode ? 'opacity-20' : 'opacity-30'} blur-xl animate-bounce`}></div>
        <div className={`absolute -bottom-4 -left-4 w-32 h-32 bg-gradient-to-r from-cyan-500 to-purple-500 rounded-full ${isDarkMode ? 'opacity-20' : 'opacity-30'} blur-xl animate-bounce delay-1000`}></div>
      </div>
